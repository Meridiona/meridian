...
export interface Stage {
  ...
  /** Show or hide the walkthrough's replica of the menu-bar popover.
   *
   *  The tray is a separate window hanging off the system menu bar, outside this
   *  one entirely — the tour cannot open it, ring it, or observe a click in it.
   *  So the off switch is taught with a drawing or not at all, and "how do I stop
   *  it recording" is the question an ambient tool has to answer before it is
   *  trusted, not after. Nothing it does is real; the beat says so. */
  demoTray(on: boolean): void
