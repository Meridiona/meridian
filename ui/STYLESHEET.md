...
Single source of truth: `ui/lib/brand-icons.ts`'s `BRAND_ICONS` map (real hex + vector wordmark per app, sourced from simple-icons/Font Awesome Free — see that file's header for provenance and how to add/refresh an entry). Resolve it only via `getBrandIcon(app)`, never a raw `BRAND_ICONS[app]` index — real macOS `app_name` values sometimes carry invisible Unicode formatting characters (WhatsApp's window title is literally a leading left-to-right mark + `"WhatsApp"`), which `getBrandIcon()` strips before matching; a raw index silently misses and falls back to a hashed hue. `TimeByApp.tsx`'s `appColor()` and `atoms.tsx`'s `AppGlyph` both go through `getBrandIcon()`. Any app still not in the map falls back to a deterministic hashed hue (`appHue()`/`appMeta()`), not a real brand color. Do not hand-maintain a second copy of this list here — it drifts (this table used to list VS Code/Jira/GitHub/Postman/Gmail/DevTools, none of which were actually in the map).

An entry may be hex-only (`{ hex }`, no `viewBox`/`path`) when no redistributable wordmark exists — `AppGlyph` falls back to the letter-monogram glyph but still uses the real brand color instead of a hashed one. ChatGPT and VS Code (`Code`) are hex-only for this reason (no OpenAI or Microsoft mark in simple-icons or Font Awesome Free); the same treatment Apple's own system apps get via `atoms.tsx`'s `APP_META`.

Currently mapped: Google Chrome `#4285F4` · Claude `#D97757` · Claude Code `#D97757` · Arc `#FCBFBD` · WhatsApp `#25D366` · ChatGPT `#000000` (hex-only) · Code/VS Code `#007ACC` (hex-only) · DBeaver `#382923` · Zoom `#0B5CFF` · Linear `#5E6AD2` · Figma `#F24E1E` · Notion `#000000` · Spotify `#1ED760` · Safari `#006CFF` · Xcode `#147EFB` · iTerm2 `#000000` · Slack `#4A154B`.
