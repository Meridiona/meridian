...
//!
//! Usage:
//!   backfill-session-categories --today
//!   backfill-session-categories --yesterday
//!   backfill-session-categories --from-date 2025-05-01 --to-date 2025-05-14
//!   backfill-session-categories --from-id 100 --to-id 500
//!   backfill-session-categories --from-id 100           # from 100 onwards
//!   backfill-session-categories --dry-run --today        # print without writing

use anyhow::{bail, Context, Result};
use chrono::{Local, NaiveDate, TimeZone, Utc};
use meridian::{
    config::{Config, LlmBackendConfig},
    intelligence::{category_llm::backends::build_backend, category_settler::settle_range},
};
...
fn print_usage() {
    eprintln!(
        "usage: backfill-session-categories [--today | --yesterday \
