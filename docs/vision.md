...
- **Local-first always.** Capture and classification run on-device, into a database encrypted at rest. Your screen content, window titles, and captured text never leave the machine. What does leave is deliberately small and enumerable: the ticket updates you approve, sent directly to the trackers you connect; redacted error-level diagnostics you can switch off; and two content-free usage events once you sign in. Privacy through architecture, not promises — which means enumerating the exceptions rather than claiming there are none.
