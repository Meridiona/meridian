...
- **Resolved**: #421 merged and deleted the 7 zero-consumer `atoms.tsx` exports (`Card`, `ConfidenceRing`, `SegBar`, `LiveDot`, `CatLabel`, `fmtDurDecimal`, `useTick`) as dead code. This package's `src/index.ts` barrel has had those 7 names dropped, their preview stories removed, and the bundle rebuilt to match.
