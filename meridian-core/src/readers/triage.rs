...
//! `/api/triage` GET ported to Rust — a faithful port of `ui/app/api/triage/route.ts`.
//!
//! # What this is
...
//! each reason code turned into a human hint. Read-only — the decision *writes*
//! live in the (not-yet-ported) `triage/{decision,ignore,apply}` routes.
//!
//! # Who calls this
//! The tray `get_triage` command ([`crate`] consumer: `tray/src-tauri`), which
//! the dashboard would invoke. NOTE: there is currently no dashboard consumer of
//! the bare GET — the cleanup page reads hygiene via `/api/tasks`
//! ([`crate::tasks`]) and only uses the triage *sub-routes*. This port exists for
//! parity with the daemon's cleanup engine and a future triage UI.
//!
//! # Related
...
//!   ALL reasons and has its own [`reason_hint`].
