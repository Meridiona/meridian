...
npm_global_writable() {
    ...
    [[ -n "$prefix" ]] || return 1
    # Check the prefix owner — the most reliable signal. `stat -f %Su` is macOS syntax.
    # If /usr/local (or any system path) is root-owned, npm install -g needs sudo.
    # -w alone is not sufficient: ACLs or missing @scope dirs can still deny access.
