...
| `MLX_IDLE_EVICT_S` | `120` (secs) | Idle-eviction window for the MLX model. The model holds ~7 GB of Metal unified memory while resident (invisible to `ps`/Activity Monitor — measure with `mx.get_active_memory()`), so the server lazy-loads it on first request and unloads it after this many seconds idle (~3s cold reload). `0` disables eviction (pins the model). See `services/agents/server.py` `_idle_evictor` + `run_task_linker_mlx.py` `maybe_evict_idle`. |
