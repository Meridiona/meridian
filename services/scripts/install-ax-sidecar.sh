...
# It polls ~/.claude/projects/ every 3 s, parses recently-modified Claude
# Code session .jsonl files, and inserts the conversation transcript into
# screenpipe's frames table (device_name='claude-session').
#
...
echo "       FROM frames WHERE device_name='claude-session' ORDER BY timestamp DESC LIMIT 5;\""
