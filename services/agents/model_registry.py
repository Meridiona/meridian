...
``session_distiller`` and the ``/prefetch_model`` route all read
...
# mlx_lm.load()'s default fileset — exactly what a generative MLX
...
class ModelSpec:
    ...
    generative weights (``mlx_lm.load``) and ``"mlx_embeddings"`` for the encoder
...
# Ordered llm → embedder: the wizard prefetches in this order, and the llm is
# largest so its progress dominates the bar first.
ALL_SPECS: tuple[ModelSpec, ...] = (LLM, EMBEDDER)
