...
"""Unit tests for Stage 2 scoring math.
...
from agents import stage2


def test_cos_to_unit_rescales_extremes():
    """cosine -1/0/+1 map to 0/0.5/1 in unit space."""
    assert stage2._cos_to_unit(-1.0) == 0.0
    assert stage2._cos_to_unit(0.0) == 0.5
    assert stage2._cos_to_unit(1.0) == 1.0


def test_cos_to_unit_clamps_out_of_range():
    """Numerical noise outside [-1,1] is clamped, not propagated."""
    assert stage2._cos_to_unit(-2.0) == 0.0
    assert stage2._cos_to_unit(2.0) == 1.0


def test_jaccard_disjoint_identical_half():
    """Jaccard returns 0 for disjoint, 1 for identical, 0.5 for half-overlap."""
    assert stage2._jaccard(set(), set()) == 0.0
    assert stage2._jaccard({"a", "b"}, {"c", "d"}) == 0.0
    assert stage2._jaccard({"a", "b"}, {"a", "b"}) == 1.0
    # {a,b} vs {a,c}: intersection {a}, union {a,b,c} → 1/3
    assert math.isclose(stage2._jaccard({"a", "b"}, {"a", "c"}), 1 / 3)
    # {a,b} vs {a,b,c,d}: intersection {a,b}, union 4 → 0.5
    assert stage2._jaccard({"a", "b"}, {"a", "b", "c", "d"}) == 0.5
...
def test_activity_match_only_on_intersection():
    ...
    assert stage2._activity_match(sess_dims, {"activity": ["coding"]}) == 1.0
    assert stage2._activity_match(sess_dims, {"activity": ["debugging"]}) == 0.0
    assert stage2._activity_match(sess_dims, None) == 0.0
    assert stage2._activity_match({}, {"activity": ["coding"]}) == 0.0
...
def test_dim_overlap_score_blends_with_documented_weights():
    ...
    score, detail = stage2._dim_overlap_score(sess_dims, expected)
...
def test_dim_overlap_score_weights_partial_overlap():
    ...
    score, _detail = stage2._dim_overlap_score(sess_dims, expected)
    assert math.isclose(score, 0.40)


def test_blend_score_full_mode_weights():
    """has_dim and has_past → 0.55 cosine + 0.30 dim + 0.15 past."""
    out = stage2._blend_score(0.8, 0.6, 0.4, has_dim=True, has_past=True)
...
def test_blend_score_cold_start_no_past():
    ...
    out = stage2._blend_score(0.8, 0.6, 0.0, has_dim=True, has_past=False)
...
def test_blend_score_cold_start_no_dim():
    ...
    out = stage2._blend_score(0.8, 0.0, 0.4, has_dim=False, has_past=True)
...
def test_blend_score_cosine_only():
    ...
    out = stage2._blend_score(0.8, 0.0, 0.0, has_dim=False, has_past=False)
...
def test_routing_decision_auto_when_high_and_separated():
    ...
    assert stage2._routing_decision(0.70, 0.60) == "auto"


def test_routing_decision_queue_when_top_passes_floor_only():
    """top1>=0.40 but gap too small → queue."""
    assert stage2._routing_decision(0.65, 0.60) == "queue"  # gap 0.05 < 0.08
    assert stage2._routing_decision(0.50, 0.10) == "queue"  # below auto floor


def test_routing_decision_skip_when_below_floor():
    """top1<0.40 → skip."""
    assert stage2._routing_decision(0.39, 0.0) == "skip"


def test_derive_expected_dims_maps_issue_types():
    """task→coding, bug→debugging, story→coding+planning, spike→research."""
    assert stage2.derive_expected_dims({"issue_type": "task", "title": "", "description_text": ""})["activity"] == ["coding"]
    assert stage2.derive_expected_dims({"issue_type": "bug",  "title": "", "description_text": ""})["activity"] == ["debugging"]
    assert stage2.derive_expected_dims({"issue_type": "spike", "title": "", "description_text": ""})["activity"] == ["research"]
    story = stage2.derive_expected_dims({"issue_type": "story", "title": "", "description_text": ""})
    ...
    assert stage2.derive_expected_dims({"issue_type": "weird", "title": "", "description_text": ""})["activity"] == ["coding"]
...
def test_derive_expected_dims_extracts_topics_and_tools_from_text():
    ...
    out = stage2.derive_expected_dims({
        "issue_type": "task",
