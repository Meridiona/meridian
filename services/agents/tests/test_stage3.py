...
"""Unit tests for Stage-3 JSON parsing and routing.

The actual LLM call is never reached — every test exercises `_parse_response`,
`_extract_json`, `_repair_truncated_json`, or `_routing_for` in isolation.
"""
from __future__ import annotations

import pytest

from agents import stage3
...
def test_parse_response_accepts_clean_json():
    ...
    key, conf, reason, err = stage3._parse_response(raw, VALID_KEYS)
    assert err is None
    assert key == "KAN-86"
    assert conf == 0.85
...
def test_parse_response_recovers_truncated_json():
    ...
    key, conf, _reason, err = stage3._parse_response(raw, VALID_KEYS)
    assert err is None
    assert key == "KAN-86"
    assert conf == 0.85
...
def test_parse_response_recovers_truncated_json_without_trailing_comma():
    ...
    # Same shape as the prompt example, but truncated mid-value rather than
    # after the comma — repairs cleanly to a valid JSON object.
    raw = '{"task_key": "KAN-86", "confidence": 0.85'
    key, conf, _reason, err = stage3._parse_response(raw, VALID_KEYS)
    assert err is None
    assert key == "KAN-86"
...
def test_parse_response_coerces_null_literals(literal):
    ...
    key, _conf, _reason, err = stage3._parse_response(raw, VALID_KEYS)
    assert err is None
    assert key is None
...
def test_routing_for_auto_at_or_above_floor():
    """confidence >= STAGE3_AUTO_FLOOR (0.65) → auto."""
    assert stage3._routing_for(0.65, "KAN-86") == "auto"
    assert stage3._routing_for(0.95, "KAN-86") == "auto"


def test_routing_for_queue_between_floors():
    """0.40 ≤ confidence < 0.65 → queue."""
    assert stage3._routing_for(0.40, "KAN-86") == "queue"
    assert stage3._routing_for(0.60, "KAN-86") == "queue"


def test_routing_for_skip_when_low_confidence():
    """confidence < 0.40 → skip."""
    assert stage3._routing_for(0.39, "KAN-86") == "skip"
    assert stage3._routing_for(0.0, "KAN-86") == "skip"


def test_routing_for_skip_when_task_key_none():
    """A null task_key always routes to skip regardless of confidence."""
    assert stage3._routing_for(0.99, None) == "skip"
...
def test_repair_truncated_json_closes_dangling_quote_then_braces():
    ...
    repaired = stage3._repair_truncated_json(partial)
...
def test_parse_response_handles_empty_input():
    ...
    key, conf, _reason, err = stage3._parse_response("", VALID_KEYS)
    assert key is None
    assert conf == 0.0
    assert err is not None
