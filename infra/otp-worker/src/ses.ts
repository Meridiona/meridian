 ...
 * `index.ts`'s `handleVerify`) — it is never used for any security decision,
 * only to word this notification. `null`/absent means "no prior email on
 * this install", which reads as a first-time sign-up.
 */
export type AccountEvent = { kind: "signed_up"; email: string } | { kind: "email_changed"; from: string; to: string };

/**
 * Decide what (if anything) happened, from `index.ts`'s `handleVerify` point
 * of view — pure, so this is unit-tested directly rather than only through a
 * full send→verify round trip. `previousEmail` is whatever the client sent
 * (already normalized by `email.ts`'s `normalizeEmail`, or `null` if
 * absent/unparseable) — purely informational, never a security input.
 * Returns `null` for a no-op re-verify of the SAME address (nothing changed,
 * nothing to notify) — this can legitimately happen if "Change email" is
 * used to re-enter the address already on file.
 */
export function resolveAccountEvent(newEmail: string, previousEmail: string | null): AccountEvent | null {
...
/** The plain-text body of the "someone signed in / changed their email" notification. */
export function buildAccountEventBody(event: AccountEvent): string {
  if (event.kind === "signed_up") {
    return `A new Meridian install just verified its email: ${event.email}`;
  }
  return `A Meridian install changed its verified email from ${event.from} to ${event.to}`;
}

/** The HTML body — same plain, no-links shape as the other two email kinds. */
export function buildAccountEventHtml(event: AccountEvent): string {
  const safeBody = escapeHtml(buildAccountEventBody(event));
  const heading = event.kind === "signed_up" ? "New sign-up" : "Email changed";
  return `<!doctype html>
<html>
  <body style="margin:0;padding:0;background-color:#f4f4f5;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:32px 16px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:12px;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
            <tr>
              <td style="padding:28px 32px;">
                <p style="margin:0 0 8px 0;font-size:15px;font-weight:600;color:#18181b;">${heading}</p>
                <p style="margin:0;font-size:13px;line-height:20px;color:#3f3f46;">${safeBody}</p>
              </td>
            </tr>
          </table>
...
/**
 * Notify `NOTIFY_EMAIL` of a sign-up or email change. Returns `false` (never
 * throws), same contract as the other two senders — this rides on
 * `ctx.waitUntil` in `index.ts`'s `handleVerify` and must never affect the
 * verify response it's attached to.
 */
export async function sendAccountEventEmail(
  event: AccountEvent,
  env: SesEnv & { NOTIFY_EMAIL: string },
  client: AwsFetcher = defaultAwsClient(env),
): Promise<boolean> {
  const subject =
    event.kind === "signed_up" ? "Meridian: new sign-up" : "Meridian: user changed their email";
  return postSesEmail(
    env.NOTIFY_EMAIL,
    subject,
    buildAccountEventBody(event),
    buildAccountEventHtml(event),
    env,
    client,
